...
Here we determine whether the data from the Portland Tide Gage supports that
idea.
...
in this setting.  We focus on examining the most recent 20 years, then explore 
other time scales.
...
So, the slope for the most recent twenty `r span` year period is lower than 13 of
the other `r span` year periods in the historic record.  Hardly a strong
...
revise that function to produce one used to automate repeat caclulations
...
functions, and served here mostly to demonstrate the logic we use.

We use non-standard evaluation ("quoting") to access the names of the .x  and .y
variables.  That way we can reference them in the data frame.  They are 
"data variables" not "environment variables".
