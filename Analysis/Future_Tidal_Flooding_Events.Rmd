...
mean lower low water (MLLW) at Portland.
...
foot, two foot, and three foot SLR scenarios (MGS used 0.8 and 1.5 foot
scenarios), and then count **days** in which flooding occurs. We focus on impact
...
over the period of record.  (See
[Tidal Flooding Events.Rmd](Graphics/Tidal Flooding Events.Rmd).  
...
that value is remarkably close to the long-term estimate of SLR at the Portland
station.
...
enough for tidal prediction) every ninteen years, and tides are predicted based
...
change in deviations one third as great as observed over the entire period of
record, at less than a mm a year.

## Distribution of Deviations
We need to figure out how best to model future deviations between observed and
predicted tide leels.  In particular, we need to identify strategies for 
creating a reasonable "random" version of deviations.  
...
certain that the mean close to zero is not by chance, but the nearly normal
distribution reflects something about the nature of atmospheric and other
non-astronomical effects on local sea level.
...
need to either use a distribution where we can tune the skewness and kurtosis,
resample from the observed distribution of deviations, or use more sophisticated
tools.
...
So, we see high autocorrelation, with a roughly 12 hour (tidal) cycle imposed
...
quasi-periodicity at weird 7 hour and 14 hour intervals, which suggests we may
need some other terms.
...
IN each Modsle simulate 1000 mock tidal epochs, and look at properties of the resulting 
...
hours 50 minutes), seasonal ARIMAs are very slow to fit. Even first first order 
seasonal terms take four or five minutes to fit.  Second order and higher terms 
take substantially longer. An exhaustve search bogged down immediately.
...
We get a qualitatively different fifth order ARIMA model if we set `stepwise` to 
`FALSE`. The alternate model (not shown) has one AR term and four MA terms.
...
THis reduces, but does not eliminate the periodic components in the ACF and 
PACF.  magnitudes are reduced by a third to a half.

Unfortunately, base R and `forcast` have no suitable functions for simulating
seasonal ARMIA models, although some other packages apparently do. WE do not
continue with exploration of simulation from an ARIMA model
...
autocorrelation structure shows a more rapid decrease in correlation, and a negative correlation pattern out about three days.  THe modeled values lack the prominent
periodic components of the real data.  There is somewhat more long-term 
structure in the observed data, but we're not far off.
...
`r round(slr_annual$ratio_1, 1)`. Interestingly, that is mostly because the
...
`These`res_auto()` is not a fully encapsulated function, since it relies on the 
existence of a dataframe called "epoch", with data columns with specific names.
```{r res_auto fxn}
...
Tee following takes a couple of minutes to run.  It simulates 1000
sets of tide levels for each of four SLR scenarios.
```{r resamples}
set.seed(12345)
samp = 1000
...
epoch.  With average SLR on the order of 2 mm per year, observed elevations
...
higher? WE can compare frequency of flooding with an extra 4 cm of base
elevation but versus flooding an additional 4cm odfplus one foor of SLR
...
number of expected flooding events by 50%, making the relative impact of
